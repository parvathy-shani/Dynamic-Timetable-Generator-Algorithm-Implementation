...
def generate_chromosome(pairs, batch):
    ...
    for pair in random_pairs:  # consider one gene from the randomly generated pool
        timeslot, room, lecturer, course, room_batch, purpose, code = extract_pair_info(
            pair)  # extract the relevant data

        if (batch == room_batch) and (code not in used_codes) and (
                timeslot == f'G{gene_number}'):  # check if the batch for which the chromosome generated is same and the gene hasn't been used yet and the timeslot matches the position
            if purpose == 'o' and (
            gene_number) % 3 == 1:  # checks whether the purpose is others (lab/project) or lecture
                ...
                for _ in range(2):
                    ...
                    new_pair = (['G' + str(gene_number)] + list(new_pair[0][1:]),
                                new_pair[1])  # Modify the gene number in the new pair
